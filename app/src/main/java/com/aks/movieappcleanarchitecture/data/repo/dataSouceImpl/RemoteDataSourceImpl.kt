...
class RemoteDataSourceImpl(private val movieApis: MovieApis) : RemoteDataSource {
    override suspend fun getBreakingNews(pageNum: Int): Response<GetBreakingNewsResponse> {
        return movieApis.getBreakingNews("in", pageNum, API_KEY)
    }

    override suspend fun getNowPlayingMovie(pageNum: Int): Response<GetNowPlayingMovieResponse> {
        return movieApis.getNowPlayingMovie(BASE_URL_MOVIE_NOW_PLAYING, API_KEY_MOVIE,pageNum)
    }

    override suspend fun getTopRatedMovie(pageNum: Int): Response<GetTopRatedMovieResponse> {
